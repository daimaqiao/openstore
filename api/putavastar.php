...
// 生成oim信息
$oim= oim_init("avastar");
$oim_size= array_key_exists("size", $_POST)? (int)$_POST["size"]: $file["size"];
$resolution= array_key_exists("resolution", $_POST)? $_POST["resolution"]: "";
oim_reset($oim, $oim_size, array("resolution"=> $resolution));
...
// 生成缩略图
$tmp_thumbnail= thumbnail_make($tmp_name, "", "avastar");
if(!$tmp_thumbnail) {
...
function get_dlurl($fileid_base64, $oim_base64= "") {
	return result_make_dlurl(get_baseurl(), "putavastar", $fileid_base64, $oim_base64);
}
